...
test('should be able to generate 5 unique numbers with low possibility space', () => {
  ...
  for (let i = 0; i <= 5; i++) {
    const result = copycat.uniqueByInput(i, method, inputStore, store, {
      attempts: 100,
    })
    results.push(result)
...
test('should be able to generate 5 unique numbers with low possibility space based on random seed', () => {
  ...
  const results: Array<number> = []
  for (let i = 0; i <= 5; i++) {
    const result = copycat.uniqueByInput(
      randomUUID(),
      method,
      inputStore,
      store,
...
test('should work with scramble method', () => {
  ...
  expect(results).toEqual([
    'zmnmq ngcsx',
    // Should preserve the two duplicates since the input is the same
    'zmnmq ngcsx',
  ...
  expect(numberOfCollisions).toBe(1)
})
...
test('should call attempsReached when no more attempts are available', () => {
  ...
  for (let i = 0; i <= 5; i++) {
    copycat.uniqueByInput(i, method, inputStore, store, {
      attempts: 1,
      attemptsReached: () => {
...
test('should rethrow too much attempts error', () => {
  ...
  store.add(5)
  expect(() =>
    copycat.uniqueByInput(1, method, inputStore, store, {
      attempts: 1,
      attemptsReached: () => {
